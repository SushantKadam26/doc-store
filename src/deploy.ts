/**
 * Deploy mn-test contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
import { resolveNetwork, getOrCreateWallet, formatWalletBackupNotice, recordDeployment } from './network.ts';
...
// The DocStore contract has two witnesses (documentContent, ownerPrivateKey),
// so the private state is seeded with a placeholder document + key. Update
// these values via the CLI before calling storeDocument.
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy mn-test to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState. The DocStore witnesses (documentContent,
      // ownerPrivateKey) read their inputs from this private state; placeholders
      // here are replaced by the CLI (npm run cli) with the user's real
      // document content and key before storing.
      // args is the contract constructor's arguments: empty for DocStore's
      // no-arg constructor. (Statically-typed contracts can omit
